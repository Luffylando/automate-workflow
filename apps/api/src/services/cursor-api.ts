import { requireCursorConfig } from "../config";
...
export async function createCloudAgent(prompt: string): Promise<{
  ...
  const { apiKey, repoUrl, model } = requireCursorConfig();

  const response = await cursorRequest<CreateAgentResponse>("/v1/agents", apiKey, {
    method: "POST",
    body: JSON.stringify({
...
export async function getRun(
  ...
  const { apiKey } = requireCursorConfig();
  return cursorRequest<CursorRun>(
